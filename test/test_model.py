...
class TestModelModule(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.n_h1 = 256 # Hidden Layer 1 size
        self.n_h2 = 256 # Hidden Layer 2 size
    ...
    def test_accuracy(self):
        ...
        c = torch.FloatTensor([[0.1, 0.9, 0.113, 0.452, 0.78], 
                               [0.5, 0.5, 0.2, 0.9, 0.7], 
                               [0.1, 0.5, 0.2, 0.9, 0.7], 
                               [0.1, 0.5, 0.99, 0.6, 0.7]])
        d = torch.tensor([1, 3, 3, 2]) 

        e = torch.FloatTensor([[0.9, 0.3, 0.113, 0.452, 0.78], 
                               [0.5, 0.8, 0.2, 0.7, 0.7], 
                               [0.1, 0.5, 0.7, 0.534, 0.3], 
                               [0.1, 0.5, 0.12, 0.6, 0.2],
                               [0.4, 0.3, 0.2, 0.1, 0.5]])
        f = torch.tensor([4, 3, 2, 1, 0]) 
        
        self.assertAlmostEqual(self.model.accuracy(a, b), 0.666667, places=5, msg='Not')
        assert isinstance(self.model.accuracy(a,b), float)
        self.model.accuracy_reset() # resetting accuracy to 0 for new inputs
        self.assertAlmostEqual(self.model.accuracy(c, d), 1.0, places=5, msg='Not')
        self.model.accuracy_reset() # resetting accuracy to 0 for new inputs
        self.assertAlmostEqual(self.model.accuracy(e, f), 0.2, places=5, msg='Not')
